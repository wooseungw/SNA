...
for (i in 1:26){
  ...
  html <- read_html(paste0("https://scholar.google.co.kr/scholar?start=",(i-1)*10,"&q=source:AAAI&hl=ko&as_sdt=0,5&as_ylo=",year,"&as_yhi=",year))
  #html <- read_html(paste0("https://scholar.google.co.kr/scholar?start=",(i-1)*10,"&hl=ko&as_sdt=2005&sciodt=0,5&cites=11977070277539609369&scipsc="))#인용논문 
  title <- html_nodes(html, "#gs_res_ccl_mid .gs_rt") %>% 
    html_text()
  title
  title <- gsub("\\[PDF]","",title)
  title <- gsub("\\[ÀÎ¿ë]","",title)
  title <- gsub("\\[C]","",title)
...
# 데이터 프레임을 CSV 파일로 저장
write.csv(final_data, "AAAI_2018.csv", row.names = FALSE)
